...
def extract_price(text, tier_name):
    ...
    if tier_name.lower() == "free" or tier_name.lower() == "personal" or tier_name.lower() == "starter":
        if 'free' in text_lower or '$0' in text:
            return "Free"
    ...
    if 'enterprise' in tier_name.lower():
        return "Contact sales"
...
def main():
    ...
    try:
        for idx in range(len(companies_df)):
            company = companies_df.iloc[idx]['company']
            url = companies_df.iloc[idx]['pricing_url']
